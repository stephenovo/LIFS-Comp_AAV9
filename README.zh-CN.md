...
| --- | --- | --- |
| `F_pack` | **硬门槛** | 包装/生产标签 |
| `F_CNS` | 默认正向奖励 | 小鼠脑与脊髓预测均值 |
| `F_SMA` | 可选敏感性路线 | 可配置脑/脊髓权重，例如 `0.30/0.70` |
| `F_liv` | 主要惩罚 | 小鼠肝脏预测 |
...
```text
F_target（默认）= 0.50 × F_brain + 0.50 × F_spinal
F_target（可选 SMA 敏感性）= 0.30 × F_brain + 0.70 × F_spinal
S        = 0.45 × F_target − 0.35 × F_liv − 0.20 × F_off
log2(SI) = F_target − F_liv
SI       = 2^(F_target − F_liv)
```

权重只是当前工作假设。已评估基线继续使用 50/50；非等权脑/脊髓权重必须显式启用。
项目用 27 组目标权重组合检查排序稳定性。联合路线的最终清单分为
**偏脊髓、偏低肝和折中型**，同时单独保留严格帕累托标记，不强迫每一条都属于前沿。
另有一条可配置的顺序路线，依次筛选脊髓、脑、肝、心和肾；它只用于敏感性比较，
当前没有晋升为联合主清单的替代方案。可用
`compare-screening-strategies` 在完全相同的预测上量化两条路线的重叠和差异。详见
[SMA 平行筛选策略](docs/SMA_PARALLEL_SCREENING.md)。
