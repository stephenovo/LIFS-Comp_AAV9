...
> 项目：LIFS-Comp_AAV9  
> 结论等级：**绿灯 - SRA 重建、Liver 外部验收、Animal 4 留出测试、多任务集成和 100 万条虚拟筛选均已完成。**
...
用公开 Liver 标签做重建一致性检验
    ↓
Animal 1–3 训练，Animal 4 生物学留出测试
    ↓
...
序列距离隔离、Animal 4 留出测试上，相比各终点最好的 Ridge/RF，将五个终点 Pearson
`r` 分别从 `0.445/0.476/0.715/0.359/0.561` 提高到
`0.527/0.546/0.778/0.439/0.622`。五模型集成进一步达到
`0.554/0.571/0.785/0.458/0.635`。100 万条虚拟筛选随后得到 6,016 条包装下界
通过序列、169 条严格帕累托序列和 30 条多样化计算候选，因此计算筛选门已完成。
