...
Runs the data-dependent audit, benchmarks, virtual screen, funnel and composition
challenge audits, control audit, and machine-readable experimental handoff without
...
mkdir -p "$OUTPUT_DIR"
echo "[1/8] Verifying external-data checksums"
aav9-sma verify-manifest "$MANIFEST" --root "$DATA_ROOT" > "$OUTPUT_DIR/manifest_verification.json"

echo "[2/8] Auditing pinned Fit4Function release"
...
echo "[5/8] Auditing gate sensitivity and residue composition"
...
echo "[6/8] Selecting the separate packaging-only composition challenge"
aav9-sma select-composition-challenge \
  "$OUTPUT_DIR/virtual_screen_ranked.csv.gz" \
  "$OUTPUT_DIR/virtual_screen_shortlist.csv" \
  "$SCREEN_CSV" \
  --output-panel "$OUTPUT_DIR/composition_challenge_panel.csv" \
  --output-summary "$OUTPUT_DIR/composition_challenge_summary.json"

echo "[7/8] Running in-sample directionality controls"
aav9-sma audit-controls "$SCREEN_CSV" "$RECONSTRUCTED" \
  --ensemble-size "$ENSEMBLE_SIZE" --max-iter "$MAX_ITER" \
  --output-controls "$OUTPUT_DIR/funnel_control_audit.csv" \
  --output-summary "$OUTPUT_DIR/funnel_control_summary.json"

echo "[8/8] Building machine-readable experimental handoff"
PYTHONPATH=src python3 scripts/build_experimental_handoff.py \
  --shortlist "$OUTPUT_DIR/virtual_screen_shortlist.csv" \
  --controls "$OUTPUT_DIR/funnel_control_audit.csv" \
